...
- `/admin` — Single admin page arranged as four labelled bands (all before the fold on desktop): **Site** (Events/Gymnasts/Scores/Clubs stat tiles), **Manage** (primary square tiles: Refresh Cache + buttons opening Upload / Users / Identity Review (conflict-count badge) / Logged-in activity dialogs), **Usage** (range tabs + auto-refresh + 5 usage stat tiles), and **Graphs** (4 compact Chart.js graphs, 4-across on xl). Upload (drag-and-drop JSON + import-from-URL + club-mapping dialog), Users (create/delete/reset + permissions), Identity Review (merge/split) and the logged-in detail log each open in a Dialog. The old `/upload`, `/admin/activity` and `/admin/users` routes are removed — admin components live in `$lib/admin/` and the shared `Dialog.svelte` supports nesting (Escape/Tab stop propagation so inner dialogs like club-mapping/add-user close independently).
...
- **Admin identity review (Phase 4)**: `GET /api/admin/identity-review` surfaces athlete-level conflicts — `similar_names`, `name_conflicts`, `id_conflicts`, `multi_id_athletes` (each with per-athlete evidence). `POST /api/admin/athletes/merge` combines two athletes; `POST /api/admin/athletes/split` (by gnz_id/event/club) breaks one into two using a fresh synthetic gnz_id + an `identity_override` token column that clustering honors until a merge clears it. The `/admin` Identity Review card replaced the old ID Reconciliation + Suggested Merges cards.
