...
│   │   │   │   ├── Activity.svelte      # Usage group: range tabs + auto-refresh + stat tiles in a rounded flex band; onData feeds the charts
│   │   │   │   ├── ActivityCharts.svelte # 4 compact Chart.js graphs (4-across band, h-44)
│   │   │   │   ├── CloudflareCharts.svelte # 3 Cloudflare Chart.js cards (requests/day, status codes, top countries) joined into the Graphs grid
...
│   │   │   ├── admin/+page.svelte      # Single admin page — Site stats / Manage tools (dialog buttons) / Server usage (stat tiles + 4 site charts) / Cloudflare (stat tiles + 9 charts); each group sits in a subtle rounded flex band, each source's tiles and charts are grouped together
