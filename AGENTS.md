...
│   │   ├── activity_log.py  # Non-blocking batched background writer for activity_logs
│   │   ├── athlete_identity.py # Athlete clustering + rebuild_athletes + build_athletes CLI
...
- **Activity tracking** — records pages + API requests made by logged-in users for admin review. Two capture paths: (1) FastAPI `log_activity` middleware (outermost, after `add_cache_control`) decodes the Bearer token and **enqueues** one `activity_logs` row per authenticated `/api/*` request — method, path, query, status, duration — to a **non-blocking batched background writer** (`app/activity_log.py`: bounded `queue.Queue` + a daemon thread that drains up to 100 rows per transaction; rows written synchronously when the writer isn't started, i.e. test contexts). The middleware never awaits a DB write, so a request's response can't stall on logging (a concurrent SQLite write/checkpoint used to hold responses for seconds under load). (2) `POST /api/track/page` beacon from `+layout.svelte` (a `$effect` watching `user` + `$page.url.pathname`, deduped with a plain `let lastTracked` keyed `username|path+search`) records page views for client-side navigations (also enqueued). The middleware skips `/api/track/page` and `/api/admin/activity*` (so admin viewing/clearing doesn't self-log) and skips anonymous traffic. Admin views/clears call `flush()` first so queued rows appear. Writes never break or delay a request. **No IP/user-agent stored.** Admin UI at `/admin/activity` (`GET /api/admin/activity?user&type&limit&offset` returns `{items, total}`, `DELETE /api/admin/activity?user` clears; both admin-only). Route guard covers `/admin` prefix, so `/admin/activity` needs no extra `requiresAuth` entry.
