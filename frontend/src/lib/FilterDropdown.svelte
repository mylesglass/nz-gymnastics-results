...
>
  ...
  <ul
    ...
    style={sheet ? "position:fixed; left:0.75rem; right:0.75rem; bottom:0.75rem; top:auto; transform:none; margin:0; max-height:60vh; width:auto; z-index:50" : ""}
    onkeydown={(e) => {
      if (e.key === "Escape") open = false;
    }}
  >
    ...
    {#each options as opt}
      ...
      <li role="none">
        <button
          ...
          aria-checked={sel}
          class="w-full text-left px-2 rounded hover:bg-base-200 flex items-center gap-2 {sheet ? 'py-2.5' : 'py-1.5'} {sel ? 'font-semibold text-primary' : ''}"
          onclick={() => toggle(opt)}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
    ...
    {#if selected.length > 0}
      <li role="none" class="border-t border-base-300 mt-1 pt-1">
        <button
          type="button"
          class="w-full text-left px-2 rounded hover:bg-base-200 text-sm {sheet ? 'py-2.5' : 'py-1.5'}"
          onclick={() => onPick([])}
        >
          Clear {label} filter
        </button>
    ...
    {/if}
    {#if count > 0}
      <li role="none" class="sticky bottom-0 bg-base-100 border-t border-base-300 mt-1 -mx-1 -mb-1 p-1">
        <button
          type="button"
          class="w-full text-center px-2 py-2.5 rounded-lg bg-primary text-primary-content font-semibold"
          onclick={() => (open = false)}
        >
          Close ({count} selected)
        </button>
      </li>
    {/if}
  </ul>
