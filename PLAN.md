...
- [ ] Parser: skip DNS / no-score passes at ingest time
  - Scoreholder emits an explicit DNS score item (`pass_final_score` decodes to `"dns"`, `d_score=0`, `e_score=10`) for every apparatus/day an athlete didn't compete on multi-day meets. The parser currently stores these as normal `long_scores` rows (`_sanitise_float("dns")` → NULL total), creating phantom 2+ pass structures. `_build_wide_row` (transformer.py) now tolerates a DNS pass-1 (a DNS pass never hides a real pass-2 score and is never averaged into vault aggregation), but future imports should stop storing them: skip score items whose decoded `pass_final_score` is not numeric in the parser emission loop (`backend/app/parser.py` ~line 575). `has_dns()` (decoder.py) is available for the check. Existing stored DNS rows intentionally left in place — harmless since the transformer handles them; a cleanup script (like `dedupe_events.py`) could delete `long_scores` rows with `pass_final_score IS NULL` if ever wanted.
  - **Files:** `backend/app/parser.py`, `backend/app/decoder.py`