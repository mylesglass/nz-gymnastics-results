...
### STEP 31: Rankings, apparatus cleanup, and polish ✅ (14–17 Aug 2026)
- [x] **National Apparatus Rankings** — `GET /api/rankings/apparatus` (`ApparatusRankingsResponse`, national permission, server-cached + single-flight) ranks gymnasts per apparatus by their best single mark in the season via the shared `_build_event_marks` helper; WAG order `VT,UB,BB,FX`, MAG `FX,PH,SR,VT,PB,HB`. New `/rankings/apparatus` page (WAG/MAG toggle, step select, apparatus radio tabs, WAG division select, Club/Region funnel filters, D-score column, Best-column tooltip, CSV/XLSX/PDF export). A stale-response `fetchToken` guard drops out-of-order responses.
- [x] **Wellington intent immediacy** — the Wellington store was fully removed: `/api/rankings/wellington` is always live-computed (~130 ms/step) so an intent toggle reflects on the very next read. Every rebuild now drops its ~11 s Wellington phase.
- [x] **Materialized-store follow-ups** — (1) the rebuild re-kick could never spawn a follow-up rebuild (`rebuild_async` skipped whenever *any* thread was alive, so a mutation landing mid-build was silently dropped) — now keyed on a `built_epoch` snapshot so the re-kick from inside `rebuild_all` actually runs; (2) **edit-freshness**: a mutation flags its event dirty and the wide read endpoints refresh that event's store rows on demand (`rebuild_event`, ~0.4s) or live-pivot it while a full rebuild holds the lock, so a just-saved inline edit is never hidden by stale prebuilt rows; `MATERIALIZE_WORKERS` env caps the worker pool. Regression tests in `test_materialize.py` + `test_admin_edit.py`.
- [x] **WAG STEP 5–6 selection cut-off** — the Wellington ranking table marks the 10th ranked gymnast with a red line (Nationals team selection cut-off).
- [x] **Un-resolvable "All-around" apparatus cleanup** — some Scoreholder exports publish AA-day apparatus passes only under aggregate result sets named `All-around`/`All-Around | Under|Over`/`Team`, so the parser stored that label in `LongScore.apparatus`. `transformer.STANDARD_APPARATUS` (the 8 real codes) is now enforced at every consumer (`_build_event_marks`, `_compute_apparatus_specialists`, `_compute_medals` — 508 phantom medals removed — and `wellington_ranking.py`); existing rows normalized with `python -m app.fix_apparatus` (`--apply`). Regression tests: `TestUnresolvableApparatus` + `test_fix_apparatus.py`.
- [x] **Merge confirm preview + repair scripts** — every admin merge now opens a read-only `merge-preview` dialog (per-event old→new name/GNZ ID cells, Wellington-intent moves, resulting URL) after the similar-names "Keep" buttons were found wired backwards (each kept the *other* athlete). Wrong merges from the old buttons are fixed with `python -m app.repair_merges` (hardcoded corrections) or reversed with `python -m app.reverse_merges` (split a merged athlete back into its pre-merge profiles, spec-derived automatically from a pre-merge backup). Both rebuild athletes so slug redirects restore original URLs. Applied to dev + production (15–16 Aug) against `results.pre-identity-fix.db`.
- [x] **Maintenance CLIs** — `dedupe_events.py` (collapse repeated re-uploads of the same event, keep the copy with the most `long_scores`), `backfill_host_club.py` (fill `host_club` on existing events without re-upload), `fix_apparatus.py` (relabel un-resolvable apparatus), all dry-run by default with `--apply`.
- [x] **Rankings remember your place** — new `frontend/src/lib/rankingState.svelte.ts` module-level `$state` (`{ discipline, selectedStep }`) shared by the National, Apparatus and Wellington ranking pages so discipline/step survive SvelteKit navigation (opening a gymnast and going back keeps your place; the existing `!steps.includes(step) → steps[0]` fallback handles year/discipline changes). Patch-note entry added.

### Next Steps
- [x] Medal counts + totals for gymnasts, clubs and regional teams
  - Gold/silver/bronze (G/S/B) medal tallies per gymnast, per club, and per regional/provincial team, aggregated from `LongScore.apparatus_rank` / `aa_rank` (1 = gold, 2 = silver, 3 = bronze) across scored ranking rows, deduped so multi-pass vaults and the AA rank duplicated across a gymnast's apparatus rows count once; every competition (including Nationals) treated the same; gymnasts keyed on `athlete_id` (name-key fallback).
  - `GET /api/medals` (cached 300s) accepts `year` / `gnz_id` / `club` / `athlete_id` / `slug` filters and returns per-gymnast and per-club G/S/B + totals (+ `nationals` breakdown).
  - Frontend: medal badges (gold/silver/bronze dots + counts) on the gymnast page, club page, gymnast list, and club list — `lib/medals.ts` + `MedalBadges.svelte`/`MedalDot.svelte`, wired via `WideResultsTable` and the SSR page loads.
  - **Files:** `backend/app/main.py`, `backend/app/schemas.py`, `backend/app/cache.py`, `frontend/src/lib/api.ts`, `frontend/src/lib/{medals.ts,MedalBadges.svelte,MedalDot.svelte}`, `frontend/src/routes/gymnast/[slug]/+page.svelte`, `frontend/src/routes/club/[club]/+page.svelte`, `frontend/src/routes/gymnasts/+page.svelte`, `frontend/src/routes/clubs/+page.svelte`, `backend/tests/test_medals.py`
...
- [x] Gymnast page Personal Bests card + season meta
  - `SeasonBest.svelte` renders at the top of `/gymnast/[slug]` when a specific year is selected (not "All"): the best score on each apparatus that season (secondary-coloured box + D-score underneath, tooltip shows the competition/round), the best all-around actually achieved (`aa-score` max; finals/day-2 rows count only when the row has a complete apparatus set, partial sums never masquerade as an AA), and the Best Possible AA (sum of the per-apparatus bests, primary-coloured box, with an explanatory tooltip). A vertical divider separates the apparatus from the Best Possible AA.
  ...
  - **Files:** `frontend/src/lib/SeasonBest.svelte` (new), `frontend/src/lib/WideResultsTable.svelte` (new optional `onData` callback + `afterHeader`/`nameBadge`/`nameMeta` snippets), `frontend/src/routes/gymnast/[slug]/+page.svelte`
